...
func (h *ProductHandler) GetProductHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Warn("ERROR parsing id: ", slog.String("err", err.Error()), slog.String("op", op))
	...
	if err != nil {
		...
		if errors.Is(err, domain.ErrProductNotFound) {
			h.log.Warn("Error getting product", slog.String("err", err.Error()), slog.String("op", op))
	...
	h.log.Info("get product: ", slog.String("id", id))
